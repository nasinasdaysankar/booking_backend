...
const getSanitizedOrderForNotify = async (orderId) => {
  ...
  return {
    ...
    roomNumber: plainOrder.roomNumber || plainOrder.room_number,
    blockName: plainOrder.blockName || plainOrder.block_name,
    receiverPhone: plainOrder.receiverPhone || plainOrder.receiver_phone,
...
export const assignPartner = async (req, res) => {
  try {
    ...
    emitDeliveryAssignment(partner.id, {
      ...
      blockName: order.blockName || order.block_name,
    ...
    try {
      ...
      if (userTokens.length > 0) {
        await sendPushNotification(
          userTokens.map(t => t.fcmToken),
          "Out for Delivery! 🛵",
          `Your order #${order.billId || order.id} is on the way.`,
          { 
            orderId: order.id.toString(), 
