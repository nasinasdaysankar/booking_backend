...
export const getTrendData = async (req, res) => {
  try {
    ...
    if (range === "daily") {
      ...
      dateExpr = `DATE_TRUNC('hour', "createdAt")`;
    }
    else if (range === "weekly") {
      dateExpr = `DATE_TRUNC('week', "createdAt")`;
    }
...
export const getOrdersOverview = async (req, res) => {
  try {
    ...
    if (range === "daily") {
      // Group by Hour for Daily view
      // Return the timestamp as the label so frontend can convert to IST
      groupExpr = `DATE_TRUNC('hour', "createdAt")`;
      labelExpr = `DATE_TRUNC('hour', "createdAt")::TEXT`;
    } else if (range === "monthly") {
      groupExpr = `DATE_TRUNC('month', "createdAt")`;
      labelExpr = `TO_CHAR("createdAt", 'Mon YYYY')`;
    } else {
