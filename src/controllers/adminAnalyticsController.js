...
export const getTrendData = async (req, res) => {
  try {
    ...
    // ✅ FIX: Daily should show single aggregated value, not hourly breakdown
    if (range === "daily") {
      dateExpr = `DATE("createdAt")`;
      // Add date filtering for daily view
      whereDate = `
        AND (
          (:from IS NULL OR "createdAt" >= :from::timestamp)
          AND (:to IS NULL OR "createdAt" <= :to::timestamp)
        )
      `;
    }
    else if (range === "weekly") {
...
export const getOrdersOverview = async (req, res) => {
  try {
    ...
    // ✅ FIX: Daily should aggregate by date, not hour
    if (range === "daily") {
      groupExpr = `DATE("createdAt")`;
      labelExpr = `TO_CHAR("createdAt", 'DD Mon')`;
      whereDate = `
...
export const getPeakHours = async (req, res) => {
  try {
    ...
    // ✅ FIX: Use from/to parameters instead of hardcoded dates
    let whereDate = `
      AND (
        (:from IS NULL OR "createdAt" >= :from::timestamp)
        AND (:to IS NULL OR "createdAt" <= :to::timestamp)
