...
export const addMenuItem = async (req, res) => {
  try {
    const { cafeteriaId, name, price, imageUrl, category, isTodaySpecial, isParcelAvailable, parcelCharges, stock, trackStock, autoStockUpdate, defaultStockQuantity } = req.body;
    ...
    const item = await MenuItem.create({
      ...
      parcelCharges: parcelCharges !== undefined ? parcelCharges : 10.00,
      stock: stock || 0,
...
export const updateMenuItem = async (req, res) => {
  try {
    const { id } = req.params;
    const { name, price, category, isAvailable, isTodaySpecial, imageUrl, isParcelAvailable, parcelCharges, stock, trackStock, autoStockUpdate, defaultStockQuantity } = req.body;
    ...
    if (parcelCharges !== undefined) item.parcelCharges = parcelCharges;
